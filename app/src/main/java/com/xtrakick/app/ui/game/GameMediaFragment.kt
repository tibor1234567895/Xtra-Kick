...
import com.xtrakick.app.databinding.FragmentGameBinding
...
class GameMediaFragment : BaseNetworkFragment(), Scrollable, FragmentHost, IntegrityDialog.CallbackListener {
    private companion object {
        const val TAB_TAG_PREFIX = "game_tab:"
    }
    ...
    override val currentFragment: Fragment?
        get() = childFragmentManager.fragments.firstOrNull {
            it.tag?.startsWith(TAB_TAG_PREFIX) == true && it.isVisible
        }
        ...
        with(binding) {
            ...
            (spinner.editText as? MaterialAutoCompleteTextView)?.apply {
                ...
                if (previousItem == -1) {
                    ...
                    showFragment(tabs, previousItem)
                }
    ...
    private fun updateGameLayout(game: Game?) {
        with(binding) {
            ...
            if (game?.broadcastersCount != null && requireContext().prefs().getBoolean(AppConstants.UI_BROADCASTERSCOUNT, true)) {
            ...
            if (!game?.tags.isNullOrEmpty() && requireContext().prefs().getBoolean(AppConstants.UI_TAGS, true)) {
    ...
    private fun showFragment(tabs: List<String>, position: Int) {
        val tag = TAB_TAG_PREFIX + (tabs.getOrNull(position) ?: position.toString())
        val transaction = childFragmentManager.beginTransaction()
        childFragmentManager.fragments
            .filter { it.tag?.startsWith(TAB_TAG_PREFIX) == true && !it.isHidden }
            .forEach(transaction::hide)
        val fragment = childFragmentManager.findFragmentByTag(tag)
        if (fragment == null) {
            transaction.add(R.id.fragmentContainer, onSpinnerItemSelected(tabs, position), tag)
        } else {
