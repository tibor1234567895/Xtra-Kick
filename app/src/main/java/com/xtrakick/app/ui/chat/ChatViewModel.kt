...
import com.xtrakick.app.model.chat.ChannelPointsSummary
...
import com.xtrakick.app.model.chat.Raid
...
import com.xtrakick.app.util.chat.StvEventApiWebSocket
...
class ChatViewModel @Inject constructor(
    ...
    private fun effectiveDelayMs(): Long {
        return when (applicationContext.prefs().getString(AppConstants.CHAT_DELAY_MODE, "0")) {
            "1" -> liveLatencyMs.value
            "2" -> applicationContext.prefs().getInt(AppConstants.CHAT_DELAY_CUSTOM_SECS, 5) * 1000L
            else -> 0L
        }
    }
    ...
    private fun getKickAccountId(): String? {
        return applicationContext.tokenPrefs().getString(AppConstants.KICK_USER_ID, null)?.takeIf { it.isNotBlank() }
            ?: applicationContext.tokenPrefs().getString(AppConstants.USER_ID, null)?.takeIf { it.isNotBlank() }
    }

    private fun getKickAccountLogin(): String? {
        return applicationContext.tokenPrefs().getString(AppConstants.KICK_USER_LOGIN, null)?.takeIf { it.isNotBlank() }
            ?: applicationContext.tokenPrefs().getString(AppConstants.USERNAME, null)?.takeIf { it.isNotBlank() }
    }
    ...
    private fun buildMutedUserKeys(users: List<com.xtrakick.app.model.ui.MutedChatUser>): Set<String> {
        return buildSet {
            users.forEach { user ->
    ...
    private fun loadEmotes(channelId: String?, channelLogin: String?) {
        ...
        val emoteQuality = applicationContext.prefs().getString(AppConstants.CHAT_IMAGE_QUALITY, "4") ?: "4"
        val animateGifs = applicationContext.prefs().getBoolean(AppConstants.ANIMATED_EMOTES, true)
    ...
    private suspend fun getKickAccessTokenForChatSend(forceRefresh: Boolean = false): String? {
        ...
        return headers[AppConstants.HEADER_TOKEN]
            ?.removePrefix("Bearer ")
            ?.trim()
    ...
    private fun isKickReplayChatDebugEnabled(): Boolean {
        return BuildConfig.DEBUG && applicationContext.prefs().getBoolean(AppConstants.DEBUG_KICK_CLIP_CHAT, false)
    }
    ...
    private fun isKickRecentChatDebugEnabled(): Boolean {
        ...
        val prefs = applicationContext.prefs()
        return prefs.getBoolean(AppConstants.DEBUG_KICK_REALTIME_CHAT, false) || prefs.getBoolean(AppConstants.DEBUG_KICK_CLIP_CHAT, false)
    }
    ...
    private fun mapKickMessages(rawMessages: List<KickMessage>): List<ChatMessage> {
        val nameDisplay = applicationContext.prefs().getString(AppConstants.UI_NAME_DISPLAY, "1")
        val showClearMsg = applicationContext.prefs().getBoolean(AppConstants.CHAT_SHOW_CLEARMSG, true)
        val showClearChat = applicationContext.prefs().getBoolean(AppConstants.CHAT_SHOW_CLEARCHAT, true)
    ...
    private fun hydrateStvChannelCosmetics(channelId: String?) {
        ...
        val prefs = applicationContext.prefs()
        val showPaints = prefs.getBoolean(AppConstants.CHAT_SHOW_PAINTS, true)
        val showBadges = prefs.getBoolean(AppConstants.CHAT_SHOW_STV_BADGES, true)
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            if (missingPaint || missingBadge) {
                runCatching {
                    ...
                    StvEventApiUtils.hydrateFromGql(defs, prefs.getBoolean(AppConstants.CHAT_USE_WEBP, true)).forEach { result ->
                        when (result) {
    ...
    fun startLiveChat(channelId: String?, channelLogin: String) {
        ...
        val isLoggedIn = com.xtrakick.app.util.AuthStateHelper.isKickLoggedIn(applicationContext)
        val showUserNotice = applicationContext.prefs().getBoolean(AppConstants.CHAT_SHOW_USERNOTICE, true)
        val showClearMsg = applicationContext.prefs().getBoolean(AppConstants.CHAT_SHOW_CLEARMSG, true)
        val showClearChat = applicationContext.prefs().getBoolean(AppConstants.CHAT_SHOW_CLEARCHAT, true)
        val notifyKickPoints = applicationContext.prefs().getBoolean(AppConstants.CHAT_POINTS_NOTIFY, true)
        val showPolls = applicationContext.prefs().getBoolean(AppConstants.CHAT_POLLS_SHOW, true)
        val showPredictions = applicationContext.prefs().getBoolean(AppConstants.CHAT_PREDICTIONS_SHOW, true)
        val nameDisplay = applicationContext.prefs().getString(AppConstants.UI_NAME_DISPLAY, "1")
        val showWebSocketDebugInfo = applicationContext.prefs().getBoolean(AppConstants.DEBUG_WEBSOCKET_INFO, false)
        ...
        val showNamePaints = applicationContext.prefs().getBoolean(AppConstants.CHAT_SHOW_PAINTS, true)
        val showStvBadges = applicationContext.prefs().getBoolean(AppConstants.CHAT_SHOW_STV_BADGES, true)
        val showPersonalEmotes = applicationContext.prefs().getBoolean(AppConstants.CHAT_SHOW_PERSONAL_EMOTES, true)
        val stvLiveUpdates = applicationContext.prefs().getBoolean(AppConstants.CHAT_STV_LIVE_UPDATES, true)
    ...
    private fun loadKickChannelPointState(
        ...
        viewModelScope.launch {
            ...
            val configuredScopes = applicationContext.prefs().getString(AppConstants.KICK_SCOPES, null)
    ...
    private fun sendKickCommand(
        ...
        val hasPublicToken = !kickPublicApiHeaders[AppConstants.HEADER_TOKEN].isNullOrBlank()
        val kickWebHeaders = KickApiHelper.getKickWebHeaders(applicationContext, true)
        fun missingAuth() {
            viewModelScope.launch {
                onMessage(ChatMessage(systemMsg = applicationContext.getString(R.string.chat_send_msg_error, applicationContext.getString(R.string.token_expired))))
