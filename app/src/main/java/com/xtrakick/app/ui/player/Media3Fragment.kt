...
class Media3Fragment : PlayerFragment() {
            ...
            val listener = object : Player.Listener {
                ...
                override fun onTracksChanged(tracks: Tracks) {
                    if (!tracks.isEmpty && !viewModel.loaded.value) {
                        ...
                        toggleSubtitles(prefs.getBoolean(AppConstants.PLAYER_SUBTITLES_ENABLED, false))
                    }
                    setSubtitlesButton()
                    if (!tracks.isEmpty) {
                                ...
                                result.addListener({
                                    if (result.get().resultCode == SessionResult.RESULT_SUCCESS) {
                                        ...
                                        val isNetworkAvailable = networkCapabilities != null
                                                ...
                                                && networkCapabilities.hasCapability(NetworkCapabilities.NET_CAPABILITY_VALIDATED)
                                        if (isNetworkAvailable) {
                                            val skipAccessToken = prefs.getString(AppConstants.TOKEN_SKIP_VIDEO_ACCESS_TOKEN, "2")?.toIntOrNull() ?: 2
                                            when {
    ...
    override fun setSubtitlesButton() {
        with(binding.playerControls) {
            ...
            if (textTracks != null && prefs.getBoolean(AppConstants.PLAYER_SUBTITLES, false)) {
                ...
                if (textTracks.isSelected) {
                    ...
                    subtitles.setOnClickListener {
                        ...
                        prefs.edit { putBoolean(AppConstants.PLAYER_SUBTITLES_ENABLED, false) }
                    }
                } else {
                    subtitles.setImageResource(androidx.media3.ui.R.drawable.exo_ic_subtitle_off)
                    subtitles.setOnClickListener {
                        toggleSubtitles(true)
                        prefs.edit { putBoolean(AppConstants.PLAYER_SUBTITLES_ENABLED, true) }
                    }
                }
            } else {
        ...
        viewModel.qualities.entries.find { it.key == selectedQuality }?.let { quality ->
            player?.let { player ->
                player.currentMediaItem?.let { mediaItem ->
                    ...
                    if (!automaticQualityChangeInProgress && ((!cellular && prefs.getString(AppConstants.PLAYER_DEFAULTQUALITY, "saved") == "saved") || (cellular && prefs.getString(AppConstants.PLAYER_DEFAULT_CELLULAR_QUALITY, "saved") == "saved"))) {
                        prefs.edit { putString(AppConstants.PLAYER_QUALITY, quality.key) }
                    }
                }
            }
        }
