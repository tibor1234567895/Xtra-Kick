...
class ReplyClickedDialog : BottomSheetDialogFragment() {
    ...
    fun newMessage(message: ChatMessage) {
        adapter?.let { adapter ->
            if ((message.reply?.threadParentId == adapter.threadParentId || message.id == adapter.threadParentId) && !message.isReply) {
                synchronized(adapter.messages) {
                    if (adapter.messages.size >= (messageLimit ?: requireContext().prefs().getInt(AppConstants.CHAT_LIMIT, 600).also { messageLimit = it })) {
    ...
    fun addMessages(messages: List<ChatMessage>) {
        adapter?.let { adapter ->
            synchronized(adapter.messages) {
                val left = (messageLimit ?: requireContext().prefs().getInt(AppConstants.CHAT_LIMIT, 600).also { messageLimit = it }) - adapter.messages.size
                if (left > 0) {
