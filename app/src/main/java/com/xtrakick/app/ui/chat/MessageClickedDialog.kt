...
import com.xtrakick.app.databinding.DialogChatMessageClickBinding
...
class MessageClickedDialog : BottomSheetDialogFragment(), IntegrityDialog.CallbackListener {
    ...
    override fun onViewCreated(view: android.view.View, savedInstanceState: Bundle?) {
        ...
        if (requireContext().prefs().getBoolean(AppConstants.DEBUG_CHAT_FULLMSG, false)) {
    ...
    private fun bindSelectedMessage(selectedMessage: ChatMessage) {
        ...
        val isKickPref = (requireContext().prefs().getString(AppConstants.API_PREFS_STREAMS, null) ?: AppConstants.DEFAULT_API_PREFS_STREAMS)
            .split(',')
            .any { item ->
                val split = item.split(':')
                split.getOrNull(0) == AppConstants.KICK && split.getOrNull(1) != "0"
            }
    ...
    fun newMessage(message: ChatMessage) {
        adapter?.let { chatAdapter ->
            if (chatAdapter.matchesSelectedUser(message) && !chatAdapter.muted) {
                synchronized(chatAdapter.messages) {
                    if (chatAdapter.messages.size >= (messageLimit ?: requireContext().prefs().getInt(AppConstants.CHAT_LIMIT, 600).also { messageLimit = it })) {
    ...
    fun addMessages(messages: List<ChatMessage>) {
        adapter?.let { chatAdapter ->
            synchronized(chatAdapter.messages) {
                val left = (messageLimit ?: requireContext().prefs().getInt(AppConstants.CHAT_LIMIT, 600).also { messageLimit = it }) - chatAdapter.messages.size
