...
class StreamDownloadWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result {
        ...
        val startWait = (context.prefs().getString(AppConstants.DOWNLOAD_STREAM_START_WAIT, "120")?.toLongOrNull())?.times(60000L)
        val endWait = (context.prefs().getString(AppConstants.DOWNLOAD_STREAM_END_WAIT, "15")?.toLongOrNull())?.times(60000L)
        val proxyHost = context.prefs().getString(AppConstants.PROXY_HOST, null)
        val proxyPort = context.prefs().getString(AppConstants.PROXY_PORT, null)?.toIntOrNull()
        val proxyMultivariantPlaylist = context.prefs().getBoolean(AppConstants.PROXY_MULTIVARIANT_PLAYLIST, false)
        val networkLibrary = context.prefs().getString(AppConstants.NETWORK_LIBRARY, "OkHttp")
        val proxyUser = context.prefs().getString(AppConstants.PROXY_USER, null)
        val proxyPassword = context.prefs().getString(AppConstants.PROXY_PASSWORD, null)
    ...
    private suspend fun download(channelLogin: String, sourceUrl: String, path: String): Boolean {
        ...
        val liveCheck = max(context.prefs().getString(AppConstants.DOWNLOAD_STREAM_LIVE_CHECK, "2")?.toLongOrNull() ?: 2L, 2L) * 1000L
        ...
        if (playlist.encrypted) {
            Log.e(TAG, "Aborting live recording: playlist is HLS-encrypted (#EXT-X-KEY); recorded segments would be unplayable")
            throw IllegalStateException("HLS playlist is encrypted (#EXT-X-KEY)")
        }
                    ...
                    launch {
                        ...
                        while (attempt <= 10) {
                            ...
                            val stream = try {
                                ...
                                if (kickPublicApiHeaders[AppConstants.HEADER_TOKEN].isNullOrBlank()) null else try {
        ...
        if (isShared) {
            ...
            while (true) {
                ...
                if (playlist.encrypted) {
                    // Stream switched to encrypted variants mid-recording: stop cleanly rather
                    // than appending ciphertext that would never play back.
                    Log.e(TAG, "Live stream became HLS-encrypted mid-recording (#EXT-X-KEY); stopping capture")
                    return true
                }
    ...
    private suspend fun startChatJob(channelLogin: String, path: String, downloadDate: Long, streamStartTime: String) {
        ...
        JsonWriter(chatFileWriter).let { writer ->
            ...
            chatReadWebSocket?.connect(CoroutineScope(Dispatchers.IO + SupervisorJob()))
        }
    }
    ...
    companion object {
        private const val TAG = "StreamDownloadWorker"
