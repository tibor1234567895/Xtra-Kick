...
import com.xtrakick.app.databinding.FragmentChannelBinding
                ...
                viewModel.stream.collectLatest { stream ->
                    if (stream != null) {
                        ...
                        if (stream.user != null) {
                            ...
                            viewModel.loadUser(requireContext().prefs().getString(AppConstants.NETWORK_LIBRARY, "OkHttp"), KickApiHelper.getKickPublicApiHeaders(requireContext()))
                        }
                    }
                }
            }
        ...
        with(binding) {
            ...
            (stream?.channelLogo ?: args.channelLogo).let {
                if (it != null) {
                    ...
                    requireArguments().putString(AppConstants.CHANNEL_PROFILEIMAGE, it)
                } else {
                    userImage.visibility = View.GONE
                }
            }
            stream?.channelName.let {
                if (it != null && it != args.channelName) {
                    ...
                    requireArguments().putString(AppConstants.CHANNEL_DISPLAYNAME, it)
                }
            }
            stream?.channelLogin.let {
                if (it != null && it != args.channelLogin) {
                    requireArguments().putString(AppConstants.CHANNEL_LOGIN, it)
                }
            }
            stream?.id.let {
                if (it != null && it != args.streamId) {
                    requireArguments().putString(AppConstants.STREAM_ID, it)
                }
            }
            ...
            if (!stream?.gameName.isNullOrBlank()) {
                ...
                gameName.setOnClickListener {
                    findNavController().navigate(
                        if (requireContext().prefs().getBoolean(AppConstants.UI_GAMEPAGER, true)) {
            ...
            if (stream?.viewerCount != null) {
                ...
                viewers.text = resources.getQuantityString(
                    R.plurals.viewers,
                    count,
                    KickApiHelper.formatCount(count, requireContext().prefs().getBoolean(AppConstants.UI_TRUNCATEVIEWCOUNT, true))
                )
            } else {
                viewers.visibility = View.GONE
            }
            if (requireContext().prefs().getBoolean(AppConstants.UI_UPTIME, true)) {
                if (stream?.startedAt != null) {
                    KickApiHelper.getUptime(stream.startedAt).let {
                        if (it != null) {
                            streamLayout.visibility = View.VISIBLE
    ...
    private fun updateUserLayout(user: User) {
        with(binding) {
            if (!userImage.isVisible && user.channelLogo != null) {
                ...
                requireArguments().putString(AppConstants.CHANNEL_PROFILEIMAGE, user.channelLogo)
            }
            ...
            if (user.createdAt != null) {
                ...
                userCreated.text = getString(R.string.created_at, KickApiHelper.formatTimeString(requireContext(), user.createdAt))
                if (user.bannerImageURL != null) {
            ...
            if (args.updateLocal) {
                viewModel.updateLocalUser(requireContext().prefs().getString(AppConstants.NETWORK_LIBRARY, "OkHttp"), requireContext().filesDir.path, user)
            }
        }
    }
