...
object KickApiHelper {
    ...
    fun getKickWebHeaders(context: Context, includeToken: Boolean = false): Map<String, String> {
        return mutableMapOf<String, String>().apply {
            ...
            if (context.prefs().getBoolean(AppConstants.ENABLE_INTEGRITY, false)) {
                context.tokenPrefs().getString(AppConstants.GQL_HEADERS, null)?.let {
                    try {
            ...
            if (!usedIntegrityHeaders) {
                KickOAuthConfig.getClientId(context)?.takeIf { it.isNotBlank() }?.let {
                    put(AppConstants.HEADER_CLIENT_ID, it)
                }
                put("Accept", "application/json, text/plain, */*")
                put("Origin", "https://kick.com")
                put("Referer", "https://kick.com/")
                put("x-kick-platform", "web")
                val kickBearerToken = AuthStateHelper.getKickBearerToken(context)
                if (!kickBearerToken.isNullOrBlank()) {
                    put(AppConstants.HEADER_TOKEN, kickBearerToken)
                }
            }
        }
    }
    ...
    fun getKickPublicApiHeaders(context: Context): Map<String, String> {
        return mutableMapOf<String, String>().apply {
            ...
            AuthStateHelper.getKickBearerToken(context)?.let {
                put(AppConstants.HEADER_TOKEN, it)
            }
        }
    }

    fun isIntegrityTokenExpired(context: Context): Boolean {
        return System.currentTimeMillis() >= context.tokenPrefs().getLong(AppConstants.INTEGRITY_EXPIRATION, 0)
    }
    ...
    fun getMessageIdString(msgId: String?): String? {
        val appContext = KickApp.INSTANCE.applicationContext
        return when (msgId) {
