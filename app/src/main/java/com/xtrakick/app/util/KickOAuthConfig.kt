...
object KickOAuthConfig {
    ...
    fun getClientId(context: Context): String? {
        val debugOverride = if (BuildConfig.DEBUG) {
            context.prefs().getString(AppConstants.KICK_DEBUG_CLIENT_ID_OVERRIDE, null)
        } else {
            null
        }
        return debugOverride?.takeIf { it.isNotBlank() }
            ?: BuildConfig.KICK_CLIENT_ID.takeIf { it.isNotBlank() }
            ?: context.prefs().getString(AppConstants.KICK_CLIENT_ID, null)?.takeIf { it.isNotBlank() }
    }

    fun getBackendBaseUrl(context: Context): String? {
        val debugOverride = if (BuildConfig.DEBUG) {
            context.prefs().getString(AppConstants.KICK_DEBUG_OAUTH_BACKEND_OVERRIDE, null)
        } else {
            null
        }
        return debugOverride?.takeIf { it.isNotBlank() }
    ...
    fun getRedirectUri(context: Context): String {
        val configured = context.prefs().getString(AppConstants.KICK_REDIRECT_URI, DEFAULT_REDIRECT_URI)
    ...
    fun getConfiguredScopes(context: Context): String {
        return context.prefs().getString(AppConstants.KICK_SCOPES, DEFAULT_SCOPES)
            ?.trim()
            ?.takeIf { it.isNotBlank() }
            ?: DEFAULT_SCOPES
    }
