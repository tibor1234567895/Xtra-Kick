...
class KickPusherChatWebSocket(
    ...
    interface Listener {
        ...
        suspend fun onChatMessage(messageJson: String) {}
            ...
            runCatching {
                ...
                when (event) {
                    ...
                    "App\\Events\\ChatMessageEvent" -> {
                        ...
                        if (dataObj != null) {
                            if (debugLogging) {
                                Log.d(tag, "chat_event channel=${root.optString("channel")}")
                            }
                            listener.onChatMessage(dataObj.toString())
                        }
                    }
                    "pusher:error" -> {
                        val dataObj = parseJsonOrNull(root.opt("data"))
                        ...
                        if (debugLogging && event.isNotBlank()) {
