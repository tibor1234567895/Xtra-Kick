package u;

import java.util.ArrayList;
import s.e;

public final class i extends n {
    public int E0 = -1;
    public int F0 = -1;
    public int G0 = -1;
    public int H0 = -1;
    public int I0 = -1;
    public int J0 = -1;
    public float K0 = 0.5f;
    public float L0 = 0.5f;
    public float M0 = 0.5f;
    public float N0 = 0.5f;
    public float O0 = 0.5f;
    public float P0 = 0.5f;
    public int Q0 = 0;
    public int R0 = 0;
    public int S0 = 2;
    public int T0 = 2;
    public int U0 = 0;
    public int V0 = -1;
    public int W0 = 0;
    public final ArrayList X0 = new ArrayList();
    public f[] Y0 = null;
    public f[] Z0 = null;

    /* renamed from: a1  reason: collision with root package name */
    public int[] f15149a1 = null;

    /* renamed from: b1  reason: collision with root package name */
    public f[] f15150b1;

    /* renamed from: c1  reason: collision with root package name */
    public int f15151c1 = 0;

    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r14v0, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r0v3, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r15v1, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r14v1, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r13v1, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r7v1, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r6v1, resolved type: int[]} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r4v2, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r2v3, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r5v1, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r6v2, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r0v9, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r2v4, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r2v5, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r2v7, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r2v8, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r0v11, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r0v13, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r0v14, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r34v0, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r14v2, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r13v2, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r7v3, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r6v5, resolved type: int[]} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r3v4, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r1v4, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r34v1, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r33v0, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r32v0, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r31v0, resolved type: int[]} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r29v0, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r3v6, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r6v6, resolved type: int[]} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r14v3, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r31v1, resolved type: int[]} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r32v1, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r29v1, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r33v1, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r34v2, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r13v5, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r6v7, resolved type: java.lang.Object[]} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r7v5, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r14v4, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r1v9, resolved type: java.lang.Object} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r9v3, resolved type: u.h} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r32v2, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r29v2, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r33v2, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r34v3, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r7v13, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r31v3, resolved type: java.lang.Object} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r33v3, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r34v4, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r14v13, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r7v21, resolved type: java.lang.Object[]} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r3v24, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r1v33, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r3v25, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r1v34, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r6v27, resolved type: int[]} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r3v26, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r31v5, resolved type: java.lang.Object} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r3v28, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r3v34, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r40v4, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r3v36, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r31v6, resolved type: java.lang.Object} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r14v14, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r4v24, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r2v57, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r3v42, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r7v25, resolved type: java.lang.Object[]} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r31v7, resolved type: java.lang.Object} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r4v26, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r14v15, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r3v44, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r7v27, resolved type: java.lang.Object[]} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r14v16, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r31v8, resolved type: java.lang.Object[]} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r32v4, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r29v4, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r33v4, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r34v5, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r22v3, resolved type: java.lang.Object} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r29v5, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r32v5, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r34v6, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r33v5, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r7v33, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r33v6, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r34v7, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r14v24, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r33v7, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r34v8, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r33v8, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r14v26, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r33v9, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r33v10, resolved type: int} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r31v10, resolved type: java.lang.Object[]} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r31v11, resolved type: java.lang.Object[]} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r31v12, resolved type: java.lang.Object[]} */
    /* JADX WARNING: type inference failed for: r1v1 */
    /* JADX WARNING: type inference failed for: r1v5 */
    /* JADX WARNING: type inference failed for: r1v7 */
    /*  JADX ERROR: JadxOverflowException in pass: RegionMakerVisitor
        jadx.core.utils.exceptions.JadxOverflowException: Regions count limit reached
        	at jadx.core.utils.ErrorsCounter.addError(ErrorsCounter.java:47)
        	at jadx.core.utils.ErrorsCounter.methodError(ErrorsCounter.java:81)
        */
    /* JADX WARNING: Multi-variable type inference failed */
    /* JADX WARNING: Removed duplicated region for block: B:213:0x0406  */
    /* JADX WARNING: Removed duplicated region for block: B:435:0x051a A[SYNTHETIC] */
    public final void R(int r37, int r38, int r39, int r40) {
        /*
            r36 = this;
            r8 = r36
            int r0 = r8.f15159s0
            r9 = 2
            r10 = 3
            r11 = 1
            r12 = 0
            if (r0 <= 0) goto L_0x0080
            u.f r0 = r8.U
            if (r0 == 0) goto L_0x0013
            u.g r0 = (u.g) r0
            v.o r0 = r0.f15126v0
            goto L_0x0014
        L_0x0013:
            r0 = 0
        L_0x0014:
            if (r0 != 0) goto L_0x0018
            r0 = 0
            goto L_0x0077
        L_0x0018:
            r2 = 0
        L_0x0019:
            int r3 = r8.f15159s0
            if (r2 >= r3) goto L_0x0076
            u.f[] r3 = r8.f15158r0
            r3 = r3[r2]
            if (r3 != 0) goto L_0x0024
            goto L_0x0073
        L_0x0024:
            boolean r4 = r3 instanceof u.j
            if (r4 == 0) goto L_0x0029
            goto L_0x0073
        L_0x0029:
            int r4 = r3.k(r12)
            int r5 = r3.k(r11)
            if (r4 != r10) goto L_0x003f
            int r6 = r3.f15115s
            if (r6 == r11) goto L_0x003f
            if (r5 != r10) goto L_0x003f
            int r6 = r3.f15116t
            if (r6 == r11) goto L_0x003f
            r6 = 1
            goto L_0x0040
        L_0x003f:
            r6 = 0
        L_0x0040:
            if (r6 == 0) goto L_0x0043
            goto L_0x0073
        L_0x0043:
            if (r4 != r10) goto L_0x0046
            r4 = 2
        L_0x0046:
            if (r5 != r10) goto L_0x0049
            r5 = 2
        L_0x0049:
            v.b r6 = r8.C0
            r6.f15487a = r4
            r6.f15488b = r5
            int r4 = r3.q()
            r6.f15489c = r4
            int r4 = r3.l()
            r6.f15490d = r4
            r0.b(r3, r6)
            int r4 = r6.f15491e
            r3.N(r4)
            int r4 = r6.f15492f
            r3.K(r4)
            int r4 = r6.f15493g
            r3.f15083b0 = r4
            if (r4 <= 0) goto L_0x0070
            r4 = 1
            goto L_0x0071
        L_0x0070:
            r4 = 0
        L_0x0071:
            r3.F = r4
        L_0x0073:
            int r2 = r2 + 1
            goto L_0x0019
        L_0x0076:
            r0 = 1
        L_0x0077:
            if (r0 != 0) goto L_0x0080
            r8.A0 = r12
            r8.B0 = r12
            r8.f15167z0 = r12
            return
        L_0x0080:
            int r13 = r8.f15165x0
            int r14 = r8.f15166y0
            int r15 = r8.f15161t0
            int r7 = r8.f15162u0
            int[] r6 = new int[r9]
            int r0 = r38 - r13
            int r0 = r0 - r14
            int r2 = r8.W0
            if (r2 != r11) goto L_0x0094
            int r0 = r40 - r15
            int r0 = r0 - r7
        L_0x0094:
            r5 = r0
            r0 = -1
            if (r2 != 0) goto L_0x00a3
            int r2 = r8.E0
            if (r2 != r0) goto L_0x009e
            r8.E0 = r12
        L_0x009e:
            int r2 = r8.F0
            if (r2 != r0) goto L_0x00af
            goto L_0x00ad
        L_0x00a3:
            int r2 = r8.E0
            if (r2 != r0) goto L_0x00a9
            r8.E0 = r12
        L_0x00a9:
            int r2 = r8.F0
            if (r2 != r0) goto L_0x00af
        L_0x00ad:
            r8.F0 = r12
        L_0x00af:
            u.f[] r0 = r8.f15158r0
            r2 = 0
            r3 = 0
        L_0x00b3:
            int r4 = r8.f15159s0
            r1 = 8
            if (r2 >= r4) goto L_0x00c6
            u.f[] r4 = r8.f15158r0
            r4 = r4[r2]
            int r4 = r4.f15095h0
            if (r4 != r1) goto L_0x00c3
            int r3 = r3 + 1
        L_0x00c3:
            int r2 = r2 + 1
            goto L_0x00b3
        L_0x00c6:
            if (r3 <= 0) goto L_0x00e1
            int r4 = r4 - r3
            u.f[] r0 = new u.f[r4]
            r2 = 0
            r4 = 0
        L_0x00cd:
            int r3 = r8.f15159s0
            if (r2 >= r3) goto L_0x00e1
            u.f[] r3 = r8.f15158r0
            r3 = r3[r2]
            int r12 = r3.f15095h0
            if (r12 == r1) goto L_0x00dd
            r0[r4] = r3
            int r4 = r4 + 1
        L_0x00dd:
            int r2 = r2 + 1
            r12 = 0
            goto L_0x00cd
        L_0x00e1:
            r12 = r0
            r8.f15150b1 = r12
            r8.f15151c1 = r4
            int r0 = r8.U0
            java.util.ArrayList r3 = r8.X0
            if (r0 == 0) goto L_0x0742
            u.e r2 = r8.K
            u.e r1 = r8.J
            u.e r10 = r8.L
            u.e r9 = r8.M
            r27 = r9
            int[] r9 = r8.f15113q0
            if (r0 == r11) goto L_0x0538
            r11 = 2
            if (r0 == r11) goto L_0x0370
            r11 = 3
            if (r0 == r11) goto L_0x0101
            goto L_0x0105
        L_0x0101:
            int r11 = r8.W0
            if (r4 != 0) goto L_0x0111
        L_0x0105:
            r31 = r6
            r32 = r7
            r29 = r13
            r33 = r14
            r34 = r15
            goto L_0x0755
        L_0x0111:
            r3.clear()
            u.h r0 = new u.h
            r19 = r3
            u.e r3 = r8.J
            r20 = r4
            u.e r4 = r8.K
            r21 = r5
            u.e r5 = r8.L
            r22 = r6
            u.e r6 = r8.M
            r16 = r0
            r23 = r1
            r1 = r36
            r24 = r2
            r2 = r11
            r28 = r10
            r10 = r19
            r29 = r13
            r13 = r20
            r30 = r21
            r31 = r22
            r32 = r7
            r7 = r30
            r0.<init>(r1, r2, r3, r4, r5, r6, r7)
            r10.add(r0)
            if (r11 != 0) goto L_0x01eb
            r1 = 0
            r2 = 0
            r3 = 0
            r7 = 0
        L_0x014b:
            if (r7 >= r13) goto L_0x01e1
            r4 = 1
            int r6 = r1 + 1
            r5 = r12[r7]
            r4 = r30
            int r16 = r8.U(r4, r5)
            int[] r1 = r5.f15113q0
            r19 = 0
            r1 = r1[r19]
            r20 = r5
            r5 = 3
            if (r1 != r5) goto L_0x0165
            int r2 = r2 + 1
        L_0x0165:
            r19 = r2
            if (r3 == r4) goto L_0x0170
            int r1 = r8.Q0
            int r1 = r1 + r3
            int r1 = r1 + r16
            if (r1 <= r4) goto L_0x0176
        L_0x0170:
            u.f r1 = r0.f15132b
            if (r1 == 0) goto L_0x0176
            r1 = 1
            goto L_0x0177
        L_0x0176:
            r1 = 0
        L_0x0177:
            if (r1 != 0) goto L_0x0182
            if (r7 <= 0) goto L_0x0182
            int r2 = r8.V0
            if (r2 <= 0) goto L_0x0182
            if (r6 <= r2) goto L_0x0182
            r1 = 1
        L_0x0182:
            if (r1 == 0) goto L_0x01bc
            u.h r5 = new u.h
            u.e r3 = r8.J
            u.e r2 = r8.K
            u.e r1 = r8.L
            u.e r0 = r8.M
            r21 = r0
            r0 = r5
            r22 = r1
            r1 = r36
            r25 = r2
            r2 = r11
            r30 = r4
            r4 = r25
            r33 = r14
            r34 = r15
            r14 = r20
            r15 = r5
            r5 = r22
            r20 = r6
            r6 = r21
            r21 = r9
            r9 = r7
            r7 = r30
            r0.<init>(r1, r2, r3, r4, r5, r6, r7)
            r15.f15144n = r9
            r10.add(r15)
            r0 = r15
            r3 = r16
            r1 = r20
            goto L_0x01d2
        L_0x01bc:
            r30 = r4
            r21 = r9
            r33 = r14
            r34 = r15
            r14 = r20
            r9 = r7
            if (r9 <= 0) goto L_0x01cf
            int r1 = r8.Q0
            int r1 = r1 + r16
            int r16 = r1 + r3
        L_0x01cf:
            r3 = r16
            r1 = 0
        L_0x01d2:
            r0.a(r14)
            int r7 = r9 + 1
            r2 = r19
            r9 = r21
            r14 = r33
            r15 = r34
            goto L_0x014b
        L_0x01e1:
            r21 = r9
            r33 = r14
            r34 = r15
            r15 = r30
            goto L_0x025f
        L_0x01eb:
            r21 = r9
            r33 = r14
            r34 = r15
            r1 = 0
            r2 = 0
            r9 = 0
        L_0x01f4:
            if (r9 >= r13) goto L_0x025c
            r14 = r12[r9]
            r15 = r30
            int r16 = r8.T(r15, r14)
            int[] r3 = r14.f15113q0
            r4 = 1
            r3 = r3[r4]
            r4 = 3
            if (r3 != r4) goto L_0x0208
            int r1 = r1 + 1
        L_0x0208:
            r19 = r1
            if (r2 == r15) goto L_0x0213
            int r1 = r8.R0
            int r1 = r1 + r2
            int r1 = r1 + r16
            if (r1 <= r15) goto L_0x0219
        L_0x0213:
            u.f r1 = r0.f15132b
            if (r1 == 0) goto L_0x0219
            r1 = 1
            goto L_0x021a
        L_0x0219:
            r1 = 0
        L_0x021a:
            if (r1 != 0) goto L_0x0225
            if (r9 <= 0) goto L_0x0225
            int r3 = r8.V0
            if (r3 <= 0) goto L_0x0225
            if (r3 >= 0) goto L_0x0225
            r1 = 1
        L_0x0225:
            if (r1 == 0) goto L_0x0243
            u.h r7 = new u.h
            u.e r3 = r8.J
            u.e r4 = r8.K
            u.e r5 = r8.L
            u.e r6 = r8.M
            r0 = r7
            r1 = r36
            r2 = r11
            r30 = r12
            r12 = r7
            r7 = r15
            r0.<init>(r1, r2, r3, r4, r5, r6, r7)
            r12.f15144n = r9
            r10.add(r12)
            r0 = r12
            goto L_0x024e
        L_0x0243:
            r30 = r12
            if (r9 <= 0) goto L_0x024e
            int r1 = r8.R0
            int r1 = r1 + r16
            int r1 = r1 + r2
            r2 = r1
            goto L_0x0250
        L_0x024e:
            r2 = r16
        L_0x0250:
            r0.a(r14)
            int r9 = r9 + 1
            r1 = r19
            r12 = r30
            r30 = r15
            goto L_0x01f4
        L_0x025c:
            r15 = r30
            r2 = r1
        L_0x025f:
            int r0 = r10.size()
            int r1 = r8.f15165x0
            int r3 = r8.f15161t0
            int r4 = r8.f15166y0
            int r5 = r8.f15162u0
            r6 = 0
            r7 = r21[r6]
            r6 = 2
            if (r7 == r6) goto L_0x0279
            r7 = 1
            r9 = r21[r7]
            if (r9 != r6) goto L_0x0277
            goto L_0x0279
        L_0x0277:
            r6 = 0
            goto L_0x027a
        L_0x0279:
            r6 = 1
        L_0x027a:
            if (r2 <= 0) goto L_0x029a
            if (r6 == 0) goto L_0x029a
            r2 = 0
        L_0x027f:
            if (r2 >= r0) goto L_0x029a
            java.lang.Object r6 = r10.get(r2)
            u.h r6 = (u.h) r6
            if (r11 != 0) goto L_0x028e
            int r7 = r6.d()
            goto L_0x0292
        L_0x028e:
            int r7 = r6.c()
        L_0x0292:
            int r7 = r15 - r7
            r6.e(r7)
            int r2 = r2 + 1
            goto L_0x027f
        L_0x029a:
            r6 = r3
            r2 = r24
            r13 = r27
            r14 = r28
            r7 = 0
            r9 = 0
            r12 = 0
            r3 = r1
            r1 = r23
        L_0x02a7:
            if (r7 >= r0) goto L_0x0368
            java.lang.Object r16 = r10.get(r7)
            r30 = r5
            r5 = r16
            u.h r5 = (u.h) r5
            if (r11 != 0) goto L_0x030c
            int r13 = r0 + -1
            if (r7 >= r13) goto L_0x02ca
            int r13 = r7 + 1
            java.lang.Object r13 = r10.get(r13)
            u.h r13 = (u.h) r13
            u.f r13 = r13.f15132b
            u.e r13 = r13.K
            r35 = r10
            r30 = 0
            goto L_0x02d2
        L_0x02ca:
            int r13 = r8.f15162u0
            r35 = r10
            r30 = r13
            r13 = r27
        L_0x02d2:
            u.f r10 = r5.f15132b
            u.e r10 = r10.M
            r16 = r5
            r17 = r11
            r18 = r1
            r19 = r2
            r20 = r14
            r21 = r13
            r22 = r3
            r23 = r6
            r24 = r4
            r25 = r30
            r26 = r15
            r16.f(r17, r18, r19, r20, r21, r22, r23, r24, r25, r26)
            int r2 = r5.d()
            int r2 = java.lang.Math.max(r9, r2)
            int r5 = r5.c()
            int r5 = r5 + r12
            if (r7 <= 0) goto L_0x0301
            int r6 = r8.R0
            int r5 = r5 + r6
        L_0x0301:
            r9 = r2
            r12 = r5
            r2 = r10
            r5 = r30
            r10 = r35
            r6 = 0
            r35 = r0
            goto L_0x0362
        L_0x030c:
            r35 = r10
            int r4 = r0 + -1
            if (r7 >= r4) goto L_0x0325
            int r4 = r7 + 1
            r10 = r35
            java.lang.Object r4 = r10.get(r4)
            u.h r4 = (u.h) r4
            u.f r4 = r4.f15132b
            u.e r4 = r4.J
            r35 = r0
            r14 = r4
            r4 = 0
            goto L_0x032d
        L_0x0325:
            r10 = r35
            int r4 = r8.f15166y0
            r35 = r0
            r14 = r28
        L_0x032d:
            u.f r0 = r5.f15132b
            u.e r0 = r0.L
            r16 = r5
            r17 = r11
            r18 = r1
            r19 = r2
            r20 = r14
            r21 = r13
            r22 = r3
            r23 = r6
            r24 = r4
            r25 = r30
            r26 = r15
            r16.f(r17, r18, r19, r20, r21, r22, r23, r24, r25, r26)
            int r1 = r5.d()
            int r1 = r1 + r9
            int r3 = r5.c()
            int r3 = java.lang.Math.max(r12, r3)
            if (r7 <= 0) goto L_0x035c
            int r5 = r8.Q0
            int r1 = r1 + r5
        L_0x035c:
            r9 = r1
            r12 = r3
            r5 = r30
            r3 = 0
            r1 = r0
        L_0x0362:
            int r7 = r7 + 1
            r0 = r35
            goto L_0x02a7
        L_0x0368:
            r0 = 0
            r31[r0] = r9
            r0 = 1
            r31[r0] = r12
            goto L_0x0755
        L_0x0370:
            r31 = r6
            r32 = r7
            r30 = r12
            r29 = r13
            r33 = r14
            r34 = r15
            r13 = r4
            r15 = r5
            int r0 = r8.W0
            int r1 = r8.V0
            if (r0 != 0) goto L_0x03a7
            if (r1 > 0) goto L_0x03a4
            r1 = 0
            r2 = 0
            r3 = 0
        L_0x0389:
            if (r1 >= r13) goto L_0x03a3
            if (r1 <= 0) goto L_0x0390
            int r4 = r8.Q0
            int r2 = r2 + r4
        L_0x0390:
            r4 = r30[r1]
            if (r4 != 0) goto L_0x0395
            goto L_0x03a0
        L_0x0395:
            int r4 = r8.U(r15, r4)
            int r4 = r4 + r2
            if (r4 <= r15) goto L_0x039d
            goto L_0x03a3
        L_0x039d:
            int r3 = r3 + 1
            r2 = r4
        L_0x03a0:
            int r1 = r1 + 1
            goto L_0x0389
        L_0x03a3:
            r1 = r3
        L_0x03a4:
            r2 = r1
            r1 = 0
            goto L_0x03c8
        L_0x03a7:
            if (r1 > 0) goto L_0x03c7
            r1 = 0
            r2 = 0
            r3 = 0
        L_0x03ac:
            if (r1 >= r13) goto L_0x03c6
            if (r1 <= 0) goto L_0x03b3
            int r4 = r8.R0
            int r2 = r2 + r4
        L_0x03b3:
            r4 = r30[r1]
            if (r4 != 0) goto L_0x03b8
            goto L_0x03c3
        L_0x03b8:
            int r4 = r8.T(r15, r4)
            int r4 = r4 + r2
            if (r4 <= r15) goto L_0x03c0
            goto L_0x03c6
        L_0x03c0:
            int r3 = r3 + 1
            r2 = r4
        L_0x03c3:
            int r1 = r1 + 1
            goto L_0x03ac
        L_0x03c6:
            r1 = r3
        L_0x03c7:
            r2 = 0
        L_0x03c8:
            int[] r3 = r8.f15149a1
            if (r3 != 0) goto L_0x03d1
            r3 = 2
            int[] r3 = new int[r3]
            r8.f15149a1 = r3
        L_0x03d1:
            if (r1 != 0) goto L_0x03d6
            r3 = 1
            if (r0 == r3) goto L_0x03da
        L_0x03d6:
            if (r2 != 0) goto L_0x03f0
            if (r0 != 0) goto L_0x03f0
        L_0x03da:
            r3 = r39
            r4 = r40
            r5 = r0
            r6 = r1
            r7 = r2
            r10 = r8
            r11 = r10
            r12 = r30
            r9 = r31
            r14 = r33
            r0 = 1
            r1 = r37
            r2 = r38
            goto L_0x050c
        L_0x03f0:
            r3 = r40
            r4 = r0
            r5 = r1
            r6 = r2
            r10 = r8
            r11 = r10
            r12 = r30
            r7 = r31
            r14 = r33
            r9 = 0
            r0 = r37
            r1 = r38
            r2 = r39
        L_0x0404:
            if (r9 != 0) goto L_0x051a
            if (r4 != 0) goto L_0x0417
            float r5 = (float) r13
            r37 = r0
            float r0 = (float) r6
            float r5 = r5 / r0
            r38 = r1
            double r0 = (double) r5
            double r0 = java.lang.Math.ceil(r0)
            int r0 = (int) r0
            r5 = r0
            goto L_0x0425
        L_0x0417:
            r37 = r0
            r38 = r1
            float r0 = (float) r13
            float r1 = (float) r5
            float r0 = r0 / r1
            double r0 = (double) r0
            double r0 = java.lang.Math.ceil(r0)
            int r0 = (int) r0
            r6 = r0
        L_0x0425:
            u.f[] r0 = r10.Z0
            if (r0 == 0) goto L_0x0432
            int r1 = r0.length
            if (r1 >= r6) goto L_0x042d
            goto L_0x0432
        L_0x042d:
            r1 = 0
            java.util.Arrays.fill(r0, r1)
            goto L_0x0437
        L_0x0432:
            r1 = 0
            u.f[] r0 = new u.f[r6]
            r10.Z0 = r0
        L_0x0437:
            u.f[] r0 = r10.Y0
            if (r0 == 0) goto L_0x0444
            int r1 = r0.length
            if (r1 >= r5) goto L_0x043f
            goto L_0x0444
        L_0x043f:
            r1 = 0
            java.util.Arrays.fill(r0, r1)
            goto L_0x0448
        L_0x0444:
            u.f[] r0 = new u.f[r5]
            r10.Y0 = r0
        L_0x0448:
            r0 = 0
        L_0x0449:
            if (r0 >= r6) goto L_0x04a6
            r1 = 0
        L_0x044c:
            if (r1 >= r5) goto L_0x049d
            int r17 = r1 * r6
            int r17 = r17 + r0
            r39 = r2
            r2 = 1
            if (r4 != r2) goto L_0x045b
            int r2 = r0 * r5
            int r17 = r2 + r1
        L_0x045b:
            r40 = r3
            r2 = r17
            int r3 = r12.length
            if (r2 < r3) goto L_0x0465
        L_0x0462:
            r17 = r9
            goto L_0x0494
        L_0x0465:
            r2 = r12[r2]
            if (r2 != 0) goto L_0x046a
            goto L_0x0462
        L_0x046a:
            int r3 = r10.U(r15, r2)
            r17 = r9
            u.f[] r9 = r10.Z0
            r9 = r9[r0]
            if (r9 == 0) goto L_0x047c
            int r9 = r9.q()
            if (r9 >= r3) goto L_0x0480
        L_0x047c:
            u.f[] r3 = r10.Z0
            r3[r0] = r2
        L_0x0480:
            int r3 = r10.T(r15, r2)
            u.f[] r9 = r10.Y0
            r9 = r9[r1]
            if (r9 == 0) goto L_0x0490
            int r9 = r9.l()
            if (r9 >= r3) goto L_0x0494
        L_0x0490:
            u.f[] r3 = r10.Y0
            r3[r1] = r2
        L_0x0494:
            int r1 = r1 + 1
            r2 = r39
            r3 = r40
            r9 = r17
            goto L_0x044c
        L_0x049d:
            r39 = r2
            r40 = r3
            r17 = r9
            int r0 = r0 + 1
            goto L_0x0449
        L_0x04a6:
            r39 = r2
            r40 = r3
            r17 = r9
            r0 = 0
            r1 = 0
        L_0x04ae:
            if (r0 >= r6) goto L_0x04c4
            u.f[] r2 = r10.Z0
            r2 = r2[r0]
            if (r2 == 0) goto L_0x04c1
            if (r0 <= 0) goto L_0x04bb
            int r3 = r10.Q0
            int r1 = r1 + r3
        L_0x04bb:
            int r2 = r10.U(r15, r2)
            int r2 = r2 + r1
            r1 = r2
        L_0x04c1:
            int r0 = r0 + 1
            goto L_0x04ae
        L_0x04c4:
            r0 = 0
            r2 = 0
        L_0x04c6:
            if (r0 >= r5) goto L_0x04dc
            u.f[] r3 = r10.Y0
            r3 = r3[r0]
            if (r3 == 0) goto L_0x04d9
            if (r0 <= 0) goto L_0x04d3
            int r9 = r10.R0
            int r2 = r2 + r9
        L_0x04d3:
            int r3 = r10.T(r15, r3)
            int r3 = r3 + r2
            r2 = r3
        L_0x04d9:
            int r0 = r0 + 1
            goto L_0x04c6
        L_0x04dc:
            r0 = 0
            r7[r0] = r1
            r0 = 1
            r7[r0] = r2
            if (r4 != 0) goto L_0x04eb
            if (r1 <= r15) goto L_0x04fd
            if (r6 <= r0) goto L_0x04fd
            int r6 = r6 + -1
            goto L_0x04f1
        L_0x04eb:
            if (r2 <= r15) goto L_0x04fd
            if (r5 <= r0) goto L_0x04fd
            int r5 = r5 + -1
        L_0x04f1:
            r0 = r37
            r1 = r38
            r2 = r39
            r3 = r40
            r9 = r17
            goto L_0x0404
        L_0x04fd:
            r1 = r37
            r2 = r38
            r3 = r39
            r9 = r31
            r31 = r7
            r7 = r6
            r6 = r5
            r5 = r4
            r4 = r40
        L_0x050c:
            r0 = r1
            r1 = r2
            r2 = r3
            r3 = r4
            r4 = r5
            r5 = r6
            r6 = r7
            r7 = r31
            r31 = r9
            r9 = 1
            goto L_0x0404
        L_0x051a:
            r37 = r0
            r38 = r1
            r39 = r2
            r40 = r3
            r0 = 1
            int[] r1 = r10.f15149a1
            r2 = 0
            r1[r2] = r6
            r1[r0] = r5
            r0 = r37
            r1 = r38
            r2 = r39
            r13 = r29
            r6 = r31
            r7 = r32
            goto L_0x0766
        L_0x0538:
            r23 = r1
            r24 = r2
            r31 = r6
            r32 = r7
            r21 = r9
            r28 = r10
            r30 = r12
            r29 = r13
            r33 = r14
            r34 = r15
            r10 = r3
            r13 = r4
            r15 = r5
            int r9 = r8.W0
            if (r13 != 0) goto L_0x0555
            goto L_0x0755
        L_0x0555:
            r10.clear()
            u.h r11 = new u.h
            u.e r3 = r8.J
            u.e r4 = r8.K
            u.e r5 = r8.L
            u.e r6 = r8.M
            r0 = r11
            r1 = r36
            r2 = r9
            r7 = r15
            r0.<init>(r1, r2, r3, r4, r5, r6, r7)
            r10.add(r11)
            r0 = 0
            r1 = 0
            r12 = 0
            if (r9 != 0) goto L_0x05d0
        L_0x0572:
            if (r12 >= r13) goto L_0x0632
            r14 = r30[r12]
            int r16 = r8.U(r15, r14)
            int[] r2 = r14.f15113q0
            r3 = 0
            r2 = r2[r3]
            r3 = 3
            if (r2 != r3) goto L_0x0584
            int r0 = r0 + 1
        L_0x0584:
            r19 = r0
            if (r1 == r15) goto L_0x058f
            int r0 = r8.Q0
            int r0 = r0 + r1
            int r0 = r0 + r16
            if (r0 <= r15) goto L_0x0595
        L_0x058f:
            u.f r0 = r11.f15132b
            if (r0 == 0) goto L_0x0595
            r0 = 1
            goto L_0x0596
        L_0x0595:
            r0 = 0
        L_0x0596:
            if (r0 != 0) goto L_0x05a3
            if (r12 <= 0) goto L_0x05a3
            int r2 = r8.V0
            if (r2 <= 0) goto L_0x05a3
            int r2 = r12 % r2
            if (r2 != 0) goto L_0x05a3
            r0 = 1
        L_0x05a3:
            if (r0 == 0) goto L_0x05bd
            u.h r11 = new u.h
            u.e r3 = r8.J
            u.e r4 = r8.K
            u.e r5 = r8.L
            u.e r6 = r8.M
            r0 = r11
            r1 = r36
            r2 = r9
            r7 = r15
            r0.<init>(r1, r2, r3, r4, r5, r6, r7)
            r11.f15144n = r12
            r10.add(r11)
            goto L_0x05c6
        L_0x05bd:
            if (r12 <= 0) goto L_0x05c6
            int r0 = r8.Q0
            int r0 = r0 + r16
            int r0 = r0 + r1
            r1 = r0
            goto L_0x05c8
        L_0x05c6:
            r1 = r16
        L_0x05c8:
            r11.a(r14)
            int r12 = r12 + 1
            r0 = r19
            goto L_0x0572
        L_0x05d0:
            if (r12 >= r13) goto L_0x0632
            r14 = r30[r12]
            int r16 = r8.T(r15, r14)
            int[] r2 = r14.f15113q0
            r3 = 1
            r2 = r2[r3]
            r7 = 3
            if (r2 != r7) goto L_0x05e2
            int r0 = r0 + 1
        L_0x05e2:
            r17 = r0
            if (r1 == r15) goto L_0x05ed
            int r0 = r8.R0
            int r0 = r0 + r1
            int r0 = r0 + r16
            if (r0 <= r15) goto L_0x05f3
        L_0x05ed:
            u.f r0 = r11.f15132b
            if (r0 == 0) goto L_0x05f3
            r0 = 1
            goto L_0x05f4
        L_0x05f3:
            r0 = 0
        L_0x05f4:
            if (r0 != 0) goto L_0x0601
            if (r12 <= 0) goto L_0x0601
            int r2 = r8.V0
            if (r2 <= 0) goto L_0x0601
            int r2 = r12 % r2
            if (r2 != 0) goto L_0x0601
            r0 = 1
        L_0x0601:
            if (r0 == 0) goto L_0x061d
            u.h r11 = new u.h
            u.e r3 = r8.J
            u.e r4 = r8.K
            u.e r5 = r8.L
            u.e r6 = r8.M
            r0 = r11
            r1 = r36
            r2 = r9
            r19 = 3
            r7 = r15
            r0.<init>(r1, r2, r3, r4, r5, r6, r7)
            r11.f15144n = r12
            r10.add(r11)
            goto L_0x0628
        L_0x061d:
            r19 = 3
            if (r12 <= 0) goto L_0x0628
            int r0 = r8.R0
            int r0 = r0 + r16
            int r0 = r0 + r1
            r1 = r0
            goto L_0x062a
        L_0x0628:
            r1 = r16
        L_0x062a:
            r11.a(r14)
            int r12 = r12 + 1
            r0 = r17
            goto L_0x05d0
        L_0x0632:
            int r1 = r10.size()
            int r2 = r8.f15165x0
            int r3 = r8.f15161t0
            int r4 = r8.f15166y0
            int r5 = r8.f15162u0
            r6 = 0
            r7 = r21[r6]
            r6 = 2
            if (r7 == r6) goto L_0x064c
            r7 = 1
            r11 = r21[r7]
            if (r11 != r6) goto L_0x064a
            goto L_0x064c
        L_0x064a:
            r6 = 0
            goto L_0x064d
        L_0x064c:
            r6 = 1
        L_0x064d:
            if (r0 <= 0) goto L_0x066d
            if (r6 == 0) goto L_0x066d
            r0 = 0
        L_0x0652:
            if (r0 >= r1) goto L_0x066d
            java.lang.Object r6 = r10.get(r0)
            u.h r6 = (u.h) r6
            if (r9 != 0) goto L_0x0661
            int r7 = r6.d()
            goto L_0x0665
        L_0x0661:
            int r7 = r6.c()
        L_0x0665:
            int r7 = r15 - r7
            r6.e(r7)
            int r0 = r0 + 1
            goto L_0x0652
        L_0x066d:
            r6 = r3
            r0 = r23
            r13 = r27
            r14 = r28
            r7 = 0
            r11 = 0
            r12 = 0
            r3 = r2
            r2 = r24
        L_0x067a:
            if (r7 >= r1) goto L_0x073b
            java.lang.Object r16 = r10.get(r7)
            r30 = r5
            r5 = r16
            u.h r5 = (u.h) r5
            if (r9 != 0) goto L_0x06df
            int r13 = r1 + -1
            if (r7 >= r13) goto L_0x069d
            int r13 = r7 + 1
            java.lang.Object r13 = r10.get(r13)
            u.h r13 = (u.h) r13
            u.f r13 = r13.f15132b
            u.e r13 = r13.K
            r35 = r10
            r30 = 0
            goto L_0x06a5
        L_0x069d:
            int r13 = r8.f15162u0
            r35 = r10
            r30 = r13
            r13 = r27
        L_0x06a5:
            u.f r10 = r5.f15132b
            u.e r10 = r10.M
            r16 = r5
            r17 = r9
            r18 = r0
            r19 = r2
            r20 = r14
            r21 = r13
            r22 = r3
            r23 = r6
            r24 = r4
            r25 = r30
            r26 = r15
            r16.f(r17, r18, r19, r20, r21, r22, r23, r24, r25, r26)
            int r2 = r5.d()
            int r2 = java.lang.Math.max(r11, r2)
            int r5 = r5.c()
            int r5 = r5 + r12
            if (r7 <= 0) goto L_0x06d4
            int r6 = r8.R0
            int r5 = r5 + r6
        L_0x06d4:
            r11 = r2
            r12 = r5
            r2 = r10
            r5 = r30
            r10 = r35
            r6 = 0
            r35 = r1
            goto L_0x0735
        L_0x06df:
            r35 = r10
            int r4 = r1 + -1
            if (r7 >= r4) goto L_0x06f8
            int r4 = r7 + 1
            r10 = r35
            java.lang.Object r4 = r10.get(r4)
            u.h r4 = (u.h) r4
            u.f r4 = r4.f15132b
            u.e r4 = r4.J
            r35 = r1
            r14 = r4
            r4 = 0
            goto L_0x0700
        L_0x06f8:
            r10 = r35
            int r4 = r8.f15166y0
            r35 = r1
            r14 = r28
        L_0x0700:
            u.f r1 = r5.f15132b
            u.e r1 = r1.L
            r16 = r5
            r17 = r9
            r18 = r0
            r19 = r2
            r20 = r14
            r21 = r13
            r22 = r3
            r23 = r6
            r24 = r4
            r25 = r30
            r26 = r15
            r16.f(r17, r18, r19, r20, r21, r22, r23, r24, r25, r26)
            int r0 = r5.d()
            int r0 = r0 + r11
            int r3 = r5.c()
            int r3 = java.lang.Math.max(r12, r3)
            if (r7 <= 0) goto L_0x072f
            int r5 = r8.Q0
            int r0 = r0 + r5
        L_0x072f:
            r11 = r0
            r0 = r1
            r12 = r3
            r5 = r30
            r3 = 0
        L_0x0735:
            int r7 = r7 + 1
            r1 = r35
            goto L_0x067a
        L_0x073b:
            r0 = 0
            r31[r0] = r11
            r0 = 1
            r31[r0] = r12
            goto L_0x0755
        L_0x0742:
            r10 = r3
            r31 = r6
            r32 = r7
            r30 = r12
            r29 = r13
            r33 = r14
            r34 = r15
            r13 = r4
            r15 = r5
            int r2 = r8.W0
            if (r13 != 0) goto L_0x0770
        L_0x0755:
            r0 = r37
            r1 = r38
            r2 = r39
            r3 = r40
            r11 = r8
            r13 = r29
            r6 = r31
            r7 = r32
            r14 = r33
        L_0x0766:
            r15 = r34
            r4 = r3
            r19 = 0
            r3 = r2
            r2 = r1
            r1 = 1
            goto L_0x07f8
        L_0x0770:
            int r0 = r10.size()
            if (r0 != 0) goto L_0x078b
            u.h r9 = new u.h
            u.e r3 = r8.J
            u.e r4 = r8.K
            u.e r5 = r8.L
            u.e r6 = r8.M
            r0 = r9
            r1 = r36
            r7 = r15
            r0.<init>(r1, r2, r3, r4, r5, r6, r7)
            r10.add(r9)
            goto L_0x07cb
        L_0x078b:
            r0 = 0
            java.lang.Object r1 = r10.get(r0)
            r9 = r1
            u.h r9 = (u.h) r9
            r9.f15133c = r0
            r1 = 0
            r9.f15132b = r1
            r9.f15142l = r0
            r9.f15143m = r0
            r9.f15144n = r0
            r9.f15145o = r0
            r9.f15146p = r0
            u.e r0 = r8.J
            u.e r1 = r8.K
            u.e r3 = r8.L
            u.e r4 = r8.M
            int r5 = r8.f15165x0
            int r6 = r8.f15161t0
            int r7 = r8.f15166y0
            int r10 = r8.f15162u0
            r16 = r9
            r17 = r2
            r18 = r0
            r19 = r1
            r20 = r3
            r21 = r4
            r22 = r5
            r23 = r6
            r24 = r7
            r25 = r10
            r26 = r15
            r16.f(r17, r18, r19, r20, r21, r22, r23, r24, r25, r26)
        L_0x07cb:
            r0 = 0
        L_0x07cc:
            if (r0 >= r13) goto L_0x07d6
            r1 = r30[r0]
            r9.a(r1)
            int r0 = r0 + 1
            goto L_0x07cc
        L_0x07d6:
            int r0 = r9.d()
            r19 = 0
            r31[r19] = r0
            int r0 = r9.c()
            r1 = 1
            r31[r1] = r0
            r0 = r37
            r2 = r38
            r3 = r39
            r4 = r40
            r11 = r8
            r13 = r29
            r6 = r31
            r7 = r32
            r14 = r33
            r15 = r34
        L_0x07f8:
            r5 = r6[r19]
            int r5 = r5 + r13
            int r5 = r5 + r14
            r6 = r6[r1]
            int r6 = r6 + r15
            int r6 = r6 + r7
            r7 = -2147483648(0xffffffff80000000, float:-0.0)
            r9 = 1073741824(0x40000000, float:2.0)
            if (r0 != r9) goto L_0x0808
            r0 = r2
            goto L_0x0814
        L_0x0808:
            if (r0 != r7) goto L_0x080f
            int r0 = java.lang.Math.min(r5, r2)
            goto L_0x0814
        L_0x080f:
            if (r0 != 0) goto L_0x0813
            r0 = r5
            goto L_0x0814
        L_0x0813:
            r0 = 0
        L_0x0814:
            if (r3 != r9) goto L_0x0818
            r2 = r4
            goto L_0x0824
        L_0x0818:
            if (r3 != r7) goto L_0x081f
            int r2 = java.lang.Math.min(r6, r4)
            goto L_0x0824
        L_0x081f:
            if (r3 != 0) goto L_0x0823
            r2 = r6
            goto L_0x0824
        L_0x0823:
            r2 = 0
        L_0x0824:
            r11.A0 = r0
            r11.B0 = r2
            r11.N(r0)
            r11.K(r2)
            int r0 = r11.f15159s0
            if (r0 <= 0) goto L_0x0833
            goto L_0x0834
        L_0x0833:
            r1 = 0
        L_0x0834:
            r11.f15167z0 = r1
            return
        */
        throw new UnsupportedOperationException("Method not decompiled: u.i.R(int, int, int, int):void");
    }

    public final int T(int i10, f fVar) {
        if (fVar == null) {
            return 0;
        }
        int[] iArr = fVar.f15113q0;
        if (iArr[1] == 3) {
            int i11 = fVar.f15116t;
            if (i11 == 0) {
                return 0;
            }
            if (i11 == 2) {
                int i12 = (int) (fVar.A * ((float) i10));
                if (i12 != fVar.l()) {
                    fVar.f15092g = true;
                    S(fVar, iArr[0], fVar.q(), 1, i12);
                }
                return i12;
            } else if (i11 == 1) {
                return fVar.l();
            } else {
                if (i11 == 3) {
                    return (int) ((((float) fVar.q()) * fVar.X) + 0.5f);
                }
            }
        }
        return fVar.l();
    }

    public final int U(int i10, f fVar) {
        if (fVar == null) {
            return 0;
        }
        int[] iArr = fVar.f15113q0;
        if (iArr[0] == 3) {
            int i11 = fVar.f15115s;
            if (i11 == 0) {
                return 0;
            }
            if (i11 == 2) {
                int i12 = (int) (fVar.f15120x * ((float) i10));
                if (i12 != fVar.q()) {
                    fVar.f15092g = true;
                    S(fVar, 1, i12, iArr[1], fVar.l());
                }
                return i12;
            } else if (i11 == 1) {
                return fVar.q();
            } else {
                if (i11 == 3) {
                    return (int) ((((float) fVar.l()) * fVar.X) + 0.5f);
                }
            }
        }
        return fVar.q();
    }

    public final void c(e eVar, boolean z10) {
        boolean z11;
        boolean z12;
        f fVar;
        float f10;
        int i10;
        boolean z13;
        super.c(eVar, z10);
        f fVar2 = this.U;
        if (fVar2 == null || !((g) fVar2).f15127w0) {
            z11 = false;
        } else {
            z11 = true;
        }
        int i11 = this.U0;
        ArrayList arrayList = this.X0;
        if (i11 != 0) {
            if (i11 == 1) {
                int size = arrayList.size();
                for (int i12 = 0; i12 < size; i12++) {
                    h hVar = (h) arrayList.get(i12);
                    if (i12 == size - 1) {
                        z12 = true;
                    } else {
                        z12 = false;
                    }
                    hVar.b(i12, z11, z12);
                }
            } else if (i11 != 2) {
                if (i11 == 3) {
                    int size2 = arrayList.size();
                    for (int i13 = 0; i13 < size2; i13++) {
                        h hVar2 = (h) arrayList.get(i13);
                        if (i13 == size2 - 1) {
                            z13 = true;
                        } else {
                            z13 = false;
                        }
                        hVar2.b(i13, z11, z13);
                    }
                }
            } else if (!(this.f15149a1 == null || this.Z0 == null || this.Y0 == null)) {
                for (int i14 = 0; i14 < this.f15151c1; i14++) {
                    this.f15150b1[i14].D();
                }
                int[] iArr = this.f15149a1;
                int i15 = iArr[0];
                int i16 = iArr[1];
                float f11 = this.K0;
                f fVar3 = null;
                int i17 = 0;
                while (i17 < i15) {
                    if (z11) {
                        i10 = (i15 - i17) - 1;
                        f10 = 1.0f - this.K0;
                    } else {
                        f10 = f11;
                        i10 = i17;
                    }
                    f fVar4 = this.Z0[i10];
                    if (!(fVar4 == null || fVar4.f15095h0 == 8)) {
                        e eVar2 = fVar4.J;
                        if (i17 == 0) {
                            fVar4.g(eVar2, this.J, this.f15165x0);
                            fVar4.f15099j0 = this.E0;
                            fVar4.f15089e0 = f10;
                        }
                        if (i17 == i15 - 1) {
                            fVar4.g(fVar4.L, this.L, this.f15166y0);
                        }
                        if (i17 > 0 && fVar3 != null) {
                            int i18 = this.Q0;
                            e eVar3 = fVar3.L;
                            fVar4.g(eVar2, eVar3, i18);
                            fVar3.g(eVar3, eVar2, 0);
                        }
                        fVar3 = fVar4;
                    }
                    i17++;
                    f11 = f10;
                }
                for (int i19 = 0; i19 < i16; i19++) {
                    f fVar5 = this.Y0[i19];
                    if (!(fVar5 == null || fVar5.f15095h0 == 8)) {
                        e eVar4 = fVar5.K;
                        if (i19 == 0) {
                            fVar5.g(eVar4, this.K, this.f15161t0);
                            fVar5.f15101k0 = this.F0;
                            fVar5.f15091f0 = this.L0;
                        }
                        if (i19 == i16 - 1) {
                            fVar5.g(fVar5.M, this.M, this.f15162u0);
                        }
                        if (i19 > 0 && fVar3 != null) {
                            int i20 = this.R0;
                            e eVar5 = fVar3.M;
                            fVar5.g(eVar4, eVar5, i20);
                            fVar3.g(eVar5, eVar4, 0);
                        }
                        fVar3 = fVar5;
                    }
                }
                for (int i21 = 0; i21 < i15; i21++) {
                    for (int i22 = 0; i22 < i16; i22++) {
                        int i23 = (i22 * i15) + i21;
                        if (this.W0 == 1) {
                            i23 = (i21 * i16) + i22;
                        }
                        f[] fVarArr = this.f15150b1;
                        if (!(i23 >= fVarArr.length || (fVar = fVarArr[i23]) == null || fVar.f15095h0 == 8)) {
                            f fVar6 = this.Z0[i21];
                            f fVar7 = this.Y0[i22];
                            if (fVar != fVar6) {
                                fVar.g(fVar.J, fVar6.J, 0);
                                fVar.g(fVar.L, fVar6.L, 0);
                            }
                            if (fVar != fVar7) {
                                fVar.g(fVar.K, fVar7.K, 0);
                                fVar.g(fVar.M, fVar7.M, 0);
                            }
                        }
                    }
                }
            }
        } else if (arrayList.size() > 0) {
            ((h) arrayList.get(0)).b(0, z11, true);
        }
        this.f15167z0 = false;
    }
}
